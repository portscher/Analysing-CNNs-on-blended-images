...
class MBConv(nn.Module):
    def __init__(self, in_, out_, expand,
                 ...
                 se_ratio, dc_ratio=0.2, attention=False, img_size=224):
        super().__init__()
        mid_ = in_ * expand
        ...
        self.project_conv = nn.Sequential(
            SamePadConv2d(mid_, out_, kernel_size=(1, 1), stride=(1, 1), bias=False),
...
class MBBlock(nn.Module):
    def __init__(self, in_, out_, expand, kernel, stride, num_repeat, skip, se_ratio, drop_connect_ratio=0.2,
                 attention=False, img_size=224):
        super().__init__()
        layers = [MBConv(in_, out_, expand, kernel, stride, skip, se_ratio, drop_connect_ratio)]
        for _ in range(1, num_repeat):
            layers.append(MBConv(out_, out_, expand, kernel, 1, skip, se_ratio, drop_connect_ratio, attention, img_size))
...
class EfficientNet(nn.Module):
    def __init__(self, width_coeff=1, depth_coeff=1,
                 ...
                 num_classes=1000, attention=False):
        super().__init__()
        ...
        self.blocks = nn.Sequential(
            ...
            MBBlock(renew_ch(24), renew_ch(40), 6, 5, 2, renew_repeat(2), True, 0.25, drop_connect_rate), # attention=attention, img_size=28
            MBBlock(renew_ch(40), renew_ch(80), 6, 3, 2, renew_repeat(3), True, 0.25, drop_connect_rate), # attention=attention, img_size=14
            MBBlock(renew_ch(80), renew_ch(112), 6, 5, 1, renew_repeat(3), True, 0.25, drop_connect_rate, attention=attention, img_size=14),
            MBBlock(renew_ch(112), renew_ch(192), 6, 5, 2, renew_repeat(4), True, 0.25, drop_connect_rate, attention=attention, img_size=7),
            MBBlock(renew_ch(192), renew_ch(320), 6, 3, 1, renew_repeat(1), True, 0.25, drop_connect_rate, attention=attention, img_size=7)
        )
